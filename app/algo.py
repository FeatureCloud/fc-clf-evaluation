...
class Client(LinearRegression):
    ...
    def local_computation(self, X, y, eps=None):
        ...
        def add_dp(matrix, epsilon=None):
            if epsilon:
                delta_12 = 1 - 1.0 / n_samples
                lambda_laplace = 3 * delta_12 / epsilon
                noise_matrix = np.random.laplace(0, lambda_laplace, size=matrix.shape)
